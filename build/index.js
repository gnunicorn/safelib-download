...
var ElectronDownloader = function () {
  ...
  _createClass(ElectronDownloader, [{
      ...
      fs.mkdirs(this.cache, function (err) {
        if (err) {
          var _ret = function () {
            ...
            var localCache = path.resolve('./.safelibs');
            return {
    ...
    value: function createTempDir(cb, onSuccess) {
      this.tmpdir = path.join(os.tmpdir(), 'safelib-tmp-download-' + process.pid + '-' + Date.now());
    ...
    key: 'baseUrl',
    get: function get() {
      return process.env.NPM_CONFIG_ELECTRON_MIRROR || process.env.npm_config_electron_mirror || process.env.ELECTRON_MIRROR || this.opts.mirror || 'https://github.com/maidsafe/safe_launcher/releases/download/';
    }
  }, {
    key: 'middleUrl',
    get: function get() {
    ...
    key: 'filename',
    get: function get() {
      return 'safe-launcher-v' + this.version + '-' + this.platform + '-' + this.arch + (this.symbols ? '-symbols' : '') + '.zip';
    }
  }, {
    key: 'platform',
    get: function get() {
